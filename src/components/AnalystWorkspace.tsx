...
Automated notification generated by eGRC Waterfall Management Portal.`;
  ...
  return (
    <div id="analyst-workspace" className="space-y-5 animate-in fade-in duration-200">
      ...
      {/* Permanent Project Details Specification (FRC Governed, Analyst View-Only) */}
      <ProjectDetailsCard
        projectDetails={projectDetails}
        userRole="analyst"
      ...
      {/* Top Header Card with Clean 3-Tab Navigation */}
      <div className="bg-white rounded-2xl border border-stone-200 p-5 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <span className="bg-emerald-100 text-emerald-800 text-[11px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                ...
                {emailDispatchedCount} of {rows.length} Email Alerts Dispatched
              </span>
            </div>
            <div className="flex items-center flex-wrap gap-2.5 mt-1">
              <h2 className="text-xl font-bold text-stone-900 tracking-tight">
                Waterfall Requirements{projectDetails.egrcNumber ? ` / ${projectDetails.egrcNumber}` : ''}
              </h2>
            </div>
            <p className="text-xs text-stone-600 mt-0.5">
              Execute <strong>Start Analytics</strong> per row, trigger automated alerts to the Project Team, and proceed to <strong>Finalise and Submit Step</strong> for PMO &amp; FRC sign-off.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl text-xs font-semibold shadow-xs transition-colors"
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 bg-stone-100 hover:bg-stone-200 text-stone-800 rounded-xl text-xs font-semibold border border-stone-300 transition-colors shadow-2xs"
        ...
        {/* 4 Clean Tabs: Waterfall Requirements Table, Complete Analytics, Finalise & Submit, and Collaborative Dispute Logs */}
        <div className="flex border-b border-stone-200 mt-5 text-xs font-semibold overflow-x-auto">
          ...
          {/* Tab 3: Finalise and Submit Step */}
          <button
            type="button"
            id="tab-finalize-submit-step"
            onClick={() => onTabChange('finalize')}
            className={`py-3 px-5 border-b-2 transition-all flex items-center gap-2 text-sm whitespace-nowrap ${
              activeTab === 'finalize'
                ? 'border-stone-900 text-stone-900 font-bold bg-stone-50/70'
                : 'border-transparent text-stone-500 hover:text-stone-900'
          ...
          >
            <Send
              className={`w-4 h-4 ${activeTab === 'finalize' ? 'text-indigo-600' : 'text-stone-400'}`}
            />
            Finalise and Submit Step
            <span className="bg-indigo-100 text-indigo-800 px-2 py-0.5 rounded text-xs font-mono font-bold">
              {finalizedCount} / {rows.length} Submitted
            </span>
          </button>

          {/* Tab 4: Collaborative Dispute & Audit Logs (Excel Replacement) */}
          <button
            type="button"
            id="tab-dispute-logs"
            onClick={() => onTabChange('dispute_logs')}
            className={`py-3 px-5 border-b-2 transition-all flex items-center gap-2 text-sm whitespace-nowrap ${
          ...
          >
            <Scale
              className={`w-4 h-4 ${activeTab === 'dispute_logs' ? 'text-indigo-600' : 'text-stone-400'}`}
            />
            Collaborative Dispute &amp; Audit Logs
            <span className="bg-indigo-100 text-indigo-800 px-2 py-0.5 rounded text-xs font-mono font-bold">
              {auditLogs.length} Events
            </span>
            <span className="bg-emerald-100 text-emerald-800 px-1.5 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wider">
              Excel Replacement
            </span>
          </button>
        </div>
      ...
      {(activeTab === 'table' || activeTab === 'start') && (
        <div className="space-y-4 animate-in fade-in duration-150">
          {/* Quick Guidance Banner & View Switcher */}
          <div className="bg-blue-50/80 border border-blue-200 rounded-xl p-3.5 flex flex-col lg:flex-row lg:items-center justify-between gap-3 text-xs text-blue-950">
            <div className="flex items-start sm:items-center gap-2.5">
              <Clock className="w-4 h-4 text-blue-600 shrink-0 mt-0.5 sm:mt-0" />
              <span>
                <strong>Analyst Execution Workflow:</strong> Every waterfall row has its own dedicated <strong>Start Analytics</strong> button in <strong>Column 2</strong> below. Click <strong>Start Analytics</strong> on any row to begin case calculations and immediately trigger the automated email alert to all project parties (PM, Analyst, and FRC Owner).
              </span>
            </div>
                      ...
                      return (
                        ...
                        <td className="py-3 px-3 align-top">
                          <div className="space-y-1">
                            ...
                            <p className="text-xs text-stone-700 leading-relaxed bg-stone-50 p-2 rounded border border-stone-200/70">
                              {row.rationale}
                            </p>
                            <span className="inline-block text-[10px] font-mono text-stone-500 bg-stone-100 px-1.5 py-0.5 rounded">
                        ...
                        <td className="py-3 px-3 align-top text-right">
                          <input
                            ...
                            id={`exclude-${row.id}`}
                            disabled={!isRowStarted || isFinalized || row.status === 'in_modification'}
                            value={vals.excludeCount}
                            onChange={(e) =>
                              handleFieldChange(row, 'excludeCount', e.target.value)
                            }
                        ...
                        <td className="py-3 px-3 align-top text-right">
                          <input
                            ...
                            disabled={!isRowStarted || isFinalized || row.status === 'in_modification'}
                            value={vals.includeCaseCount}
                            onChange={(e) =>
                              handleFieldChange(row, 'includeCaseCount', e.target.value)
                            }
                            className={`w-full text-right px-2.5 py-1.5 font-mono font-bold text-xs rounded-lg ${
                              !isRowStarted
                                ? 'bg-stone-100 text-stone-400 border border-stone-200 cursor-not-allowed'
                        ...
                        <td className="py-3 px-3 align-top text-right">
                          <input
                            ...
                            disabled={!isRowStarted || isFinalized || row.status === 'in_modification'}
                            value={vals.includeUniqueAccountCount}
                            onChange={(e) =>
                              handleFieldChange(row, 'includeUniqueAccountCount', e.target.value)
                            }
                            className={`w-full text-right px-2.5 py-1.5 font-mono font-bold text-xs rounded-lg ${
                              !isRowStarted
                                ? 'bg-stone-100 text-stone-400 border border-stone-200 cursor-not-allowed'
                          ...
                          <div className="space-y-1.5 bg-stone-50 p-2 rounded-xl border border-stone-200">
                            <div className="flex items-center justify-center gap-1">
                              <input
                                ...
                                id={`days-${row.id}`}
                                disabled={!isRowStarted || isFinalized || row.status === 'in_modification'}
                                value={vals.workingDays}
                                onChange={(e) =>
                                  handleFieldChange(row, 'workingDays', e.target.value)
                                }
                          ...
                          <div className="flex flex-col gap-1.5 min-w-[155px]">
                            ...
                            {row.status === 'in_modification' ? (
                              ...
                              <div className="space-y-1.5">
                                ...
                                <button
                                  type="button"
                                  id={`btn-view-email-${row.id}`}
                                  onClick={() => setActiveEmailRow(row)}
                                  className="w-full py-1 px-1.5 text-[10px] font-semibold text-blue-700 hover:text-blue-900 bg-blue-50 hover:bg-blue-100 rounded-md border border-blue-200 flex items-center justify-center gap-1 transition-colors"
                                  title="View email alert dispatched to Project Team"
                                >
              ...
              return (
                ...
                >
                  <div className="space-y-2.5">
                    ...
                    <div>
                      <h4 className="font-bold text-stone-900 text-sm">{row.stepTitle}</h4>
          ...
          <div className="pt-6 border-t border-stone-200 space-y-4">
            ...
            <DisputeResolutionCards
              ...
              auditLogs={auditLogs}
              userRole="analyst"
              onOpenAuditLogs={() => onTabChange('dispute_logs')}
      ...
      {activeTab === 'complete' && (
        <div className="space-y-5 animate-in fade-in duration-150">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            ...
            <div className="bg-white p-4 rounded-2xl border border-stone-200 shadow-xs">
              <span className="text-xs text-stone-500 font-semibold block">Final In-Scope Cases</span>
              <div className="flex items-baseline gap-2 mt-1">
                <span className="text-2xl font-bold font-mono text-emerald-600">
                  {rows[rows.length - 1]?.includeCaseCount?.toLocaleString() ||
                    totalIncluded.toLocaleString()}
                </span>
                <TrendingUp className="w-4 h-4 text-emerald-500" />
              </div>
              <span className="text-[11px] text-stone-400 mt-1 block">Final testing population</span>
            </div>

            <div className="bg-white p-4 rounded-2xl border border-stone-200 shadow-xs">
              <span className="text-xs text-stone-500 font-semibold block">Unique Client Accounts</span>
              <div className="flex items-baseline gap-2 mt-1">
            ...
            <div className="bg-white p-4 rounded-2xl border border-stone-200 shadow-xs">
              <span className="text-xs text-stone-500 font-semibold block">Execution Status</span>
              <div className="flex items-baseline gap-2 mt-1">
              ...
              <span className="text-[11px] text-stone-400 mt-1 block">Finalized &amp; submitted steps</span>
            </div>
          </div>
